...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.querySelector("#navbar")) {
        navTl.from("#navbar", { y: -90, opacity: 0, duration: 0.8, delay: startDelay });
    }
    if (document.querySelector(".logo-anim")) {
        navTl.from(".logo-anim", { scale: 0.8, opacity: 0, duration: 0.6 }, "-=0.4");
    }
    if (document.querySelectorAll(".nav-link").length > 0) {
        navTl.from(".nav-link", { y: -20, opacity: 0, duration: 0.5, stagger: 0.08 }, "-=0.4");
    }
    ...
    const heroSubtitles = document.querySelectorAll(".z-10 .hero-subtitle, .z-10 .hero-cta-group, .z-10 .hero-phone");
    ...
    if (productCards.length > 0) {
        const productsTrigger = document.querySelector("#popular-products")
    ...
    if (categoryCards.length > 0) {
        const categoriesTrigger = document.querySelector("#categories")
        ...
        if (categoriesTrigger) {
            categoryAnim.scrollTrigger = { trigger: categoriesTrigger, start: "top 80%" };
        }
    ...
    if (document.querySelectorAll(".testimonial-card").length > 0) {
        ...
        const testimonialsTrigger = document.querySelector("#testimonials")
        ...
        if (testimonialsTrigger) {
            testimonialTo.scrollTrigger = { trigger: testimonialsTrigger, start: "top 80%" };
        }
    ...
    if (heroStage) {
        ...
        const hspEyebrow = sidePanel ? sidePanel.querySelector('.hsp-eyebrow') : null;
        ...
        if (hspAdd) {
            hspAdd.addEventListener('click', () => {
                ...
                if (typeof gsap !== 'undefined') {
                    gsap.to(hspAdd, { scale: 0.88, duration: 0.1, yoyo: true, repeat: 1 });
                }
                ...
                if (cb && typeof gsap !== 'undefined') {
                    gsap.fromTo(cb, { scale: 1 }, { scale: 1.4, duration: 0.2, yoyo: true, repeat: 1, ease: "back.out(3)" });
                }
            });
        }
        ...
        if (heroSlides.length > 1) {
            ...
            let currentHeroSlide = Array.from(heroSlides).findIndex(s => s.classList.contains('active'));
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', (e) => {
            ...
            const newUser = { name, email, phone, password, discount: 10, discountCode: 'BIENVENIDA10' };
            users.push(newUser);
            localStorage.setItem('grupolloUsers', JSON.stringify(users));
            localStorage.setItem('grupolloCurrentUser', JSON.stringify({ name, email, phone, discount: 10, discountCode: 'BIENVENIDA10' }));

            showToast(`🎉 Cuenta creada! Tienes 10% de descuento. Cupón: BIENVENIDA10`, 'success');
            applyDiscount();
            closeLogin();
            updateLoginButton();
            registerForm.reset();
    ...
    function handleUserMenu() {
        const user = JSON.parse(localStorage.getItem('grupolloCurrentUser'));
        if (user && confirm(`Sesión activa: ${user.name}\n\n¿Deseas cerrar sesión?`)) {
...
var SEASONAL_THEMES_CONFIG = {
    mundial: {
        banner: '⚽ ¡Copa del Mundo 2026 • USA · México · Canadá! Celebra con el mejor pollo 🇲🇽🏆',
...
function applySeasonalTheme() {
    ...
    if (banner) {
        ...
        gsap.from(banner, { y: -50, opacity: 0, duration: 1, ease: 'back.out(1.7)', delay: 0.3 });
    ...
    if (theme.decos) {
        theme.decos.forEach(function(deco) {
            ...
            el.style.cssText = 'left:' + deco.left + ';top:' + deco.top
                + ';animation-delay:' + deco.delay + ';font-size:' + deco.size + ';';
    ...
    if (heroTag) {
        ...
        gsap.from(heroTag, { x: -40, opacity: 0, duration: 0.9, ease: 'back.out(1.5)', delay: 1.2 });
    }
}
...
function initScrollReveals() {
    document.querySelectorAll('[data-reveal]').forEach(function(el) {
        ...
        switch (dir) {
            case 'left':   from.x = -110; break;
            case 'right':  from.x =  110; break;
            case 'up':     from.y =  90;  break;
            case 'down':   from.y = -60;  break;
            case 'rotate': from.rotation = -20; from.scale = 0.75; break;
            case 'zoom':   from.scale = 0.5; break;
            case 'flip-y': from.rotationY = -90; from.transformPerspective = 800; break;
        }
...
function initPageLoader() {
    ...
    tl.from(logo,   { scale: 0.6, opacity: 0, duration: 0.6, ease: 'back.out(1.8)' }, 0)
...
if (seasonalBannerEl) {
    ...
    seasonalObserver.observe(seasonalBannerEl, { attributes: true, attributeFilter: ['class', 'style'] });
}
...
(function() {
    ...
    function updateCarousel() {
        ...
        slides.forEach((slide, idx) => {
            if (idx === currentIndex && slide.classList.contains('mundial-highlight-ring')) {
...
// ================================================================
window.addEventListener('scroll', () => {
    const bgGeometricElements = document.querySelectorAll('.bg-mundial-geometric');
